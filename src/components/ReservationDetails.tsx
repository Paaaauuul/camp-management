...
import { Reservation, Customer, Site } from '../types';
import { updateCustomer, updateReservation, getSites } from '../lib/queries';
...
export function ReservationDetails({ reservation, customer, onClose, onUpdate }: ReservationDetailsProps) {
  const createdAt = parseISO(reservation.created_at);
  const startDate = parseISO(reservation.start_date);
  const endDate = parseISO(reservation.end_date);
  const [sites, setSites] = React.useState<Site[]>([]);
  const [formData, setFormData] = React.useState({
    site_id: reservation.site_id.toString(),
    amount_owed: reservation.amount?.toString() || '0.00',
    amount_paid: '0.00',
  ...
  // Calculate amount owed and amount paid
  const amountOwed = reservation.amount || 0;
  const amountPaid = 0; // This would come from a payment tracking system
          ...
          <div className="p-6 border-b border-gray-200">
            <div className="flex justify-between items-start mb-4">
              <h2 className="text-lg sm:text-xl font-semibold">
                Reservation for {reservation.customers ? `${reservation.customers.first_name} ${reservation.customers.last_name}` : ''}
              </h2>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-4 text-sm text-gray-600">
              <div className="flex items-center gap-2">
                <MapPin className="h-4 w-4" /> Site {reservation.site_id}
              </div>
              <div className="flex items-center gap-2">
