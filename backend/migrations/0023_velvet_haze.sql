...
SELECT 
  ...
  'tent',
  CASE WHEN generate_series <= 2 THEN true ELSE false END,
  CASE WHEN generate_series <= 2 THEN true ELSE false END,
  'grass',
  CASE 
    WHEN generate_series <= 2 THEN 'Tent site with water and electric'
    ELSE 'Primitive tent site'
  END
...
SELECT 
  ...
  50,
  'concrete',
  'Large mobile home site with full hookups'
ON CONFLICT (name) DO NOTHING;