/*
  # Add Site Types and Additional Fields

  1. Changes to Sites Table
    - Add site_type enum ('tent', 'rv', 'mobile_home')
    - Add amenities columns:
      - has_water (boolean)
      - has_electricity (boolean)
      - has_sewer (boolean)
    - Add specifications:
      - max_length (integer) - for RV length in feet
      - max_width (integer) - for RV width in feet
      - amp_service (integer) - electrical service (30 or 50 amp)
      - pad_type (text) - surface type (gravel, concrete, etc)
...
-- Create index for site type
CREATE INDEX idx_sites_type ON sites (site_type);

-- Add constraint for amp service based on site type
ALTER TABLE sites
...
-- Add constraint for dimensions based on site type
ALTER TABLE sites
